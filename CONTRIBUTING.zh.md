...
2. **一句中文说明，且是点击理由。** `blurb` 只允许一个中文句号，且必须在句末。模式：给谁看 + 能得到什么 + 有何限制。不要堆功能清单，不要把英文原文整段贴进来。禁止用「说明如何」「文档写明」「介绍如何」开头。仓库名、英文标题要配中文译名（写进 `title` 或 `aliases`），不要把 `rakazo` 这类 slug 当唯一标题。
